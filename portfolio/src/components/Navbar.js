...
const Navbar = ({ activeSection, smoothScrollTo }) => {
    ...
    return (
        <>
            ...
            >
                <div className={`pointer-events-auto transition-all duration-300 ${isScrolled ? 'bg-black/50 backdrop-blur-xl border border-white/10 shadow-lg rounded-full px-6 py-3' : 'bg-transparent px-0 py-4'}`}>
                    <div className="flex items-center gap-8">
                        <a
                            href="#home"
                            className={`text-xl font-bold font-mono tracking-tighter transition-colors ${isScrolled ? 'text-white' : 'text-white/90'}`}
                        >
                            UB.
                        </a>
                            ...
                            {navItems.map((item) => (
                                <li key={item}>
                                    <button
                                        ...
                                        className={`text-3xl font-bold transition-colors ${activeSection === item.toLowerCase() ? 'text-white' : 'text-gray-500 hover:text-white'}`}
                                    >
                                        {item}
                                    </button>
                                </li>
